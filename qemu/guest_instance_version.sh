...
if [[ ${action} == run ]]; then
    ...
    -boot menu=on,strict=on \
    \
    -device virtio-scsi-ccw,id=virtio-scsi${instance},devno=fe.0.0008 \
    -device scsi-cd,bus=virtio-scsi${instance}.0,channel=0,scsi-id=${instance},lun=${instance},device_id=drive-scsi-cd${instance},id=scsi-cd${instance} \
    \
    -netdev tap,helper=/usr/local/libexec/qemu-bridge-helper,id=tap-netdev${instance},br=virbr0 \
    -device virtio-net-ccw,netdev=tap-netdev${instance},id=virtio-net${instance},devno=fe.0.0001 \
    \
    -blockdev file,filename=${disk},node-name=blk_storage${instance} \
    -blockdev qcow2,file=blk_storage${instance},node-name=blk_format${instance} \
    -device virtio-blk-ccw,devno=fe.0.0002,drive=blk_format${instance},id=virtio-blk${instance},bootindex=1 \
    \
    -device virtio-serial-ccw,id=virtio-serial${instance},devno=fe.0.0003 \
    -chardev stdio,id=char-stdio${instance},mux=on,signal=on,server=on,wait=off \
    -device virtserialport,bus=virtio-serial${instance}.0,nr=1,chardev=char-stdio${instance},id=virtserial${instance},name=org.qemu.guest_agent.${instance} \
    \
    -device virtio-balloon-ccw,id=virtio-balloon${instance},devno=fe.0.0005 \
    \
    -object rng-random,id=rng-rand${instance},filename=/dev/urandom \
    -device virtio-rng-ccw,rng=rng-rand${instance},id=virtio-rng${instance},devno=fe.0.0004 \
    \
    -mon chardev=char-stdio${instance},id=virt-monitor,mode=control \
    \
    -audiodev id=virtaudio${instance},driver=none \
    \
    -device zpci,uid=0,fid=0,target=vfio-pci${instance},id=zpci${instance} \
    -device vfio-pci,host=0000:00:00.1,id=vfio-pci${instance},vf-token=e3f6d79f-afd5-499f-b0da-811a2c3207a0 \
    -msg timestamp=on \
    -D ${logfile}
fi
